import {Util, Point, Canvas, LineString} from 'maptalks'
...
const canvasExtend = {
	...
	getSmoothLineParams: function(points, smoothValue, out_curveLengths, out_pointCurvatures) {
		...
		for (let i = 0; i < l; i++) {
			const x1 = extPoints[i].x, y1 = extPoints[i].y;
		...
		for(let i=0; i<points.length; i++){
			...
			(out_curveLengths && i!=points.length-1) && out_curveLengths.push(
					getCubicCurveLength(preCtrlPt[0], preCtrlPt[1], nextCtrlPt[0], nextCtrlPt[1], points[i+1].x, points[i+1].y));
...
const LineStringExtend = {
    ...
    _patintTrendArrow: function(ctx, points, lineWidth, lineColor, lineOpacity){
		...
		Canvas.paintSmoothLineNoBeginPath(ctx, rightOffsetPts, this.options['smoothness'], false);

		for(let i=0; i<headPts.length; i++)
            ctx.lineTo(headPts[i].x, headPts[i].y);
        
		reversePts(leftOffsetPts);
        Canvas.paintSmoothLineNoBeginPath(ctx, leftOffsetPts, this.options['smoothness'], false);
    ...
    _getOffsetCurvePoints: function(points, pointCurvatures, curveLengths, maxOffset, _leftOffsetPts, _rightOffsetPts){
        ...
        let totalCurveLen = 0;
        let tempLen = 0;
        for(let i=0; i<curveLengths.length; i++){     // not closed, curveNum is n-1 
            totalCurveLen += curveLengths[i];
        }

        function ifOnDirection(refvec, vec){
            if(Math.abs(Math.atan(vec.y/vec.x)-Math.atan(refvec.y/refvec.x)) <= Math.PI)
                return true;
            return false;
        }
